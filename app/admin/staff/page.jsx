...
const Staff = () => {
  ...
  // Date range state for filtering users by createdAt date
  ...
  // Filter staff based on role and createdAt date
  const filteredUsers = users.filter((user) => {
    // Check that user role is either "admin" or "support"
    const isStaff = user.role === "admin" || user.role === "support";

    // Ensure that user.createdAt is within the selected date range
    const createdAtDate = new Date(user.createdAt);
    const inDateRange =
      createdAtDate >= dateRange.from && createdAtDate <= dateRange.to;

    return isStaff && inDateRange;
  });
      ...
      <div className="flex flex-col">
        <div className="flex justify-between items-center">
          ...
          <Button variant="export" size="lg" onClick={() => setAddingUser(true)}>
            <span className="text-md">+ Add Staff</span>
          </Button>
        </div>
            ...
            {filteredUsers.length > 0 ? (
              filteredUsers.map((user) => (
                <TableRow key={user._id}>
                  ...
                  <TableCell className="flex gap-2 whitespace-nowrap px-4 py-4">
                    <Button
                      size="sm"
                      variant="export"
